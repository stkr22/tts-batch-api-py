...
This module provides Valkey-based caching for synthesized audio data
...
# AIDEV-NOTE: cache-config; centralized caching configuration for Valkey TTL management
@dataclass
class CacheConfig:
...
# AIDEV-NOTE: performance-critical; Valkey caching for audio synthesis results
class TTSCache:
    """High-performance Valkey-based cache for synthesized audio data.
    ...
    Args:
        valkey_url: Valkey connection URL with optional authentication
        config: Cache configuration including TTL and enable/disable flag

    """

    def __init__(self, valkey_url: str, config: CacheConfig) -> None:
        ...
        """
        self.valkey = aiovalkey.from_url(valkey_url)
        self.config = config
    ...
    def _generate_cache_key(self, text: str, voice_id: str = "default") -> str:
        ...
        The 'tts:' prefix helps with Valkey key organization and debugging.
    ...
    async def get(self, text: str, voice_id: str = "default") -> bytes | None:
        ...
        cached_data: bytes = await self.valkey.get(cache_key)  # type: ignore[assignment]
    ...
    async def set(self, text: str, audio_data: bytes, voice_id: str = "default") -> None:
        ...
        Uses Valkey SETEX for atomic set-with-expiration operation.
        ...
        cache_key = self._generate_cache_key(text, voice_id)
        await self.valkey.setex(cache_key, self.config.ttl, audio_data)
        self.logger.info(
