...
class ModelConfig(BaseSettings):
    ...
    # Model configuration
    available_models: Annotated[list[str], Field(
        default=["en_US-kathleen-low", "en_US-ryan-medium"],
        description="List of available PIPER TTS models to download and use"
    )]
    
    default_model: Annotated[str, Field(
        default="en_US-kathleen-low",
        description="Default model to use when no model is specified in requests"
    )]
    
    # Directory configuration  
    assets_dir: Annotated[pathlib.Path, Field(
        default=pathlib.Path("/app/assets"),
